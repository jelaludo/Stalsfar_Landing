...
export function createLandingAudio(signal) {
 ...
 for(const kind of ['launch','crash','impact'])raw[kind]=fetch(new URL(`${kind}.mp3`,base),{signal}).then(r=>r.ok?r.arrayBuffer():null).catch(()=>null);
 ...
 const play=async(kind,volume,rate=1)=>{
  if(!context||disposed||paused)return;
  const token=generation,launchToken=launchGeneration,buffer=await buffers[kind];
  ...
  source.onended=()=>{voices.delete(voice);source.disconnect();gain.disconnect();};source.start();
 };
 const thrust=value=>{
