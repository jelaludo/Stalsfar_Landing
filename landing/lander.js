...
function createView(canvas,ui,director) {
 ...
 const draw=(dt,absoluteTime,alpha=1)=>{
  ...
  const targetScale=arrival?lerp(Math.min(12,width/38),wide,pullback):flightScale;
  const targetX=arrival?v.x+(700-v.x)*wide/targetScale*pullback:v.x+(700-v.x)*wide/targetScale*(1-zoom)+v.vx*.3*zoom;
  ...
  if(state.phase!=='resolved'&&state.phase!=='done'){drawReentry(v,arrival?1:state.phase==='flying'?clamp(1-(state.time-state.entryTime)/CONFIG.cinematic.reentryTime,0,1):0,absoluteTime);drawVehicle(v,null,absoluteTime);}
...
export function runLandingIntro(canvas,options={}) {
 ...
 return new Promise(resolve=>{
  ...
  try {
   ...
   director=createDirector(options.seed??20260907,options.entries??6,options.assists);const {state}=director;
   ...
   ui.innerHTML=`<header class="masthead"><a class="wordmark" href="./">STÅLSFÄR<span>ENTRY OPERATIONS</span></a><div class="run-id">SECTOR 07 / HUGIN RECOVERY<br><span>LOCAL FLIGHT EXPERIMENT · 2D</span></div><div class="header-actions"><button data-action="pause" aria-label="Pause flight">Ⅱ <span>PAUSE</span></button><button data-action="settings" aria-expanded="false">ASSISTS</button><button data-action="skip">SKIP ↗</button></div></header>
    ...
    <div class="site-note"><span class="dot"></span> ONE SITE. SIX CHANCES.<span class="site-sub">RECOVER 5 TO REBUILD THE LAUNCHER</span></div>
   ...
   const action=name=>{
    if(name==='start'&&state.phase==='ready'){sounds.begin();clearInputs();state.phase=reduced?'flying':'arrival';state.arrivalTime=0;syncPhase();canvas.focus();}
    if(name==='skip-cinematic'){clearInputs();state.phase=state.phase==='opening'?'ready':'flying';syncPhase();(state.phase==='ready'?ui.querySelector('[data-action="start"]'):canvas).focus();}
    ...
    try{
     ...
     if(!paused&&state.phase==='opening'){director.step({},elapsed);syncPhase();acc=0;}else if(!paused&&state.phase!=='ready'){
      ...
      let steps=0;while(acc>=CONFIG.sim.dt&&steps<CONFIG.sim.maxSteps){
       ...
       sounds.thrust(state.phase==='flying'&&input.thrust&&state.v.fuel>0);
       const beforePhase=state.phase;const r=director.step(input);
       if(state.phase!==beforePhase){syncPhase();if(beforePhase==='arrival'||beforePhase==='opening'){clearInputs();(state.phase==='ready'?ui.querySelector('[data-action="start"]'):canvas).focus();}}
