...
import { createFleetDirector } from './fleet.js?v=hectic07';
import { createLaunchPlume } from './launch-plume.js';
import { createStarshipIntro, openingVariant } from './starship-intro.js';
...
export function finalApproachFrame(vehicle,fleet,width){
 const nearby=fleet.filter(u=>u.v.index!==vehicle.index&&(u.resolved||Math.abs(u.v.y-vehicle.y)<100)).sort((a,b)=>Math.abs(a.v.x-vehicle.x)-Math.abs(b.v.x-vehicle.x))[0];
 const neighbour=nearby&&Math.abs(nearby.v.x-vehicle.x)<250?nearby.v:null;
 const span=neighbour?Math.abs(neighbour.x-vehicle.x):50;
 return {scale:Math.min(CONFIG.view.closeScale,width/(span+60)),x:neighbour?(vehicle.x+neighbour.x)/2:vehicle.x};
}
function createView(canvas,ui,director) {
 ...
 let launchEffect=!reduced&&state.phase==='opening'?(state.openingVariant==='starship'?createStarshipIntro(ui):createLaunchPlume(ui)):null;
 ...
 const draw=(dt,absoluteTime,alpha=1)=>{
  ...
  let targetScale=arrival&&state.mode==='fleet'?wide:arrival?lerp(Math.min(12,width/38),wide,pullback):state.mode==='fleet'&&(state.controlled===null||state.time<.6)?wide:flightScale;
  let targetX=arrival&&state.mode==='fleet'?700:arrival?v.x+(700-v.x)*wide/targetScale*pullback:state.mode==='fleet'&&(state.controlled===null||state.time<.6)?700:v.x+(700-v.x)*wide/targetScale*(1-zoom)+v.vx*.3*zoom;
  if(state.mode==='fleet'&&state.controlled!==null&&!arrival){const close=finalApproachFrame(v,state.fleet,width),q=clamp(1-agl/130,0,1),blend=reduced?(agl<100?1:0):q*q*(3-2*q);targetScale=lerp(targetScale,Math.min(close.scale,usable*.85/(agl+25)),blend);targetX=lerp(targetX,close.x,blend);}
  if(lastIndex!==state.index){scale=wide;camX=700;camY=330;lastIndex=state.index;}
  const ease=reduced?1:1-Math.exp(-dt*10);scale=lerp(scale,targetScale,ease);camX=lerp(camX,targetX,ease);camY=t.height(observing?700:v.x)+(playBottom-playCenter-30)/scale;
  ...
  for(const unit of state.fleet??[]){if(unit.resolved||unit.v===state.v)continue;const other={...unit.v};if(arrival){const lead=(CONFIG.cinematic.arrival-state.arrivalTime)/2.2;other.x-=other.vx*lead;other.y-=other.vy*lead;}drawReentry(other,arrival?.6:clamp(1-state.time/2.8,0,.95),absoluteTime);drawVehicle(other,null,absoluteTime);const p=pt(other.x,other.y);label(`H-${other.index+1}`,clamp(p.x+8,8,width-90),clamp(p.y,playTop+50,playBottom-20),'#7bbaa4',9);}
  if(state.phase!=='resolved'&&state.phase!=='done'&&state.v.alive){drawReentry(v,arrival?1:state.phase==='flying'?clamp(1-(state.mode==='fleet'?state.time:state.time-state.entryTime)/(state.mode==='fleet'?2.8:CONFIG.cinematic.reentryTime),0,1):0,absoluteTime);drawVehicle(v,null,absoluteTime);}
...
export function runLandingIntro(canvas,options={}) {
 ...
 return new Promise(resolve=>{
  ...
  try {
   ...
   const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;state.phase=reduced?'ready':'opening';state.openingVariant=openingVariant();
   ui=document.createElement('div');ui.className='cockpit';
   ui.innerHTML=`<span class="build-badge">${state.mode==='fleet'?'HECTIC FLEET 07 · SIMULTANEOUS':'CLASSIC · SEQUENTIAL'}</span><header class="masthead"><a class="wordmark" href="./">STÅLSFÄR<span>ENTRY OPERATIONS</span></a><div class="run-id">SECTOR 07 / HUGIN RECOVERY<br><span>LOCAL FLIGHT EXPERIMENT · 2D</span></div><div class="header-actions"><button data-action="pause" aria-label="Pause flight">Ⅱ <span>PAUSE</span></button><button data-action="settings" aria-expanded="false">ASSISTS</button><button data-action="skip">SKIP ↗</button></div></header>
    ...
    <section class="start-card" hidden><div class="start-content"><span class="eyebrow">STÅLHEART / ARRIVAL SEQUENCE</span><h1>SIX DOWN<span>Bring them home.</span></h1><p>Pick a landing site. Arrest your fall.<br>Every rocket you save becomes a beginning.</p><nav class="mode-picker" aria-label="Game mode"><a href="?mode=classic">CLASSIC / SIX MANUAL</a><a href="?mode=fleet">FLEET / MANUAL OVERRIDE</a></nav><p class="mode-description">${state.mode==='fleet'?'HECTIC FLEET 07 — Six at once. Crossing trajectories. Shuffled landing sites. Observe the approach. Stand by for a control failure.':'Six consecutive descents. You pilot every booster.'}</p><p class="touch-instructions">LEFT THUMB: touch and drag to steer.<br>RIGHT THUMB: hold BOOST to brake.</p><div class="start-instructions"><span><kbd>SPACE</kbd><b>HOLD TO THRUST</b><small>Release to cut the engine</small></span><span><kbd>A / D</kbd><b>ROTATE</b><small>← / → also work</small></span><span><kbd>ESC</kbd><b>PAUSE</b><small>Press again to resume</small></span><span><kbd>1–7</kbd><b>CHOOSE A SITE</b><small>L: legs · O: retrograde</small></span></div></div><div class="start-actions"><button type="button" data-install>INSTALL APP</button><small data-install-help hidden></small><button class="primary" data-action="start">BEGIN DESCENT <span>↘</span></button></div><small>Touch: hold BURN + a rotation button together.<br>W / ↑ also thrust. Skip intro is in the menu.</small></section>
   ...
   canvas.insertAdjacentElement('afterend',ui);ui.dataset.intro=state.openingVariant;
   if(state.openingVariant==='starship'){const credit=ui.querySelector('.shader-credit');credit.href='https://github.com/pulkitxm/claude-directory/tree/main/shaders/starship-shader';credit.textContent='STARSHIP · XORDEV / PULKIT · MIT';}
   view=createView(canvas,ui,director);
   ...
   const syncPhase=()=>{
    ...
    ui.querySelector('[data-cinematic-title]').textContent=state.phase==='opening'?(state.openingVariant==='starship'?'HUGIN / SIX-STARSHIP DEPARTURE':'HUGIN / EXHAUST SIGNATURE'):'HUGIN / ATMOSPHERIC ENTRY';
   };syncPhase();
